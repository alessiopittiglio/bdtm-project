...
def get_chroma_collection(
    ...
    if not client:
        logger.error(
            "Error: ChromaDB client not provided to get_or_create_chroma_collection."
        )
        return None
    try:
        ef = None
        if embedding_model_name:
            try:
                ...
                logger.info(
                    f"Embedding function for Chroma '{embedding_model_name}' set."
                )
...
def retrieve_from_collection(
    ...
    if not collection:
        logger.error(
            "Error: ChromaDB collection not provided to retrieve_from_collection."
        )
        return None
    ...
    include_params = (
        include_fields if include_fields else ["documents", "metadatas", "distances"]
    )
